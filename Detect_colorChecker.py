...
def paralell_search(search_scale, degree, gray, H_card, W_card, card):
    ...
    if degree is not 0:
        ...
        gray_rot = imutils.rotate(gray, degree)
...
# this function searches for a pattern ('card') in the image ('img').
...
def detect_card(img, card, search_scale, search_degree, num_cores):
    ...
    if deg is not 0:
        output_img = imutils.rotate(img, deg)
    else:
        output_img = img
    # crop Colorcard
    ...
    TotSum = (Rsum + Gsum + Bsum)/float(output_img.shape[0]*output_img.shape[1])
    return(output_img,maxVal,SCALE, deg, TotSum, startX, startY,endX, endY)
